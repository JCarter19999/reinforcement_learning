...
class BanditBase:
    """
    Base class for bandit algorithms. Section 2.2: 10-armed testbed.
    """
    def __init__(
            self,
            k_arms = 10,
        ...
        self.possible_actions = np.arange(self.k_arms)

        self.reset()

    def reset(self):
        #p21: for each bandit the actiion values q_*(a) are selected according to a normal distribution with mean 0 and variance 1.

        self.q_true = np.random.randn(self.k_arms) + self.true_q_mean

        # Init 'simple bandit algo' pg. 24
        self.q_estimate = np.zeros(self.k_arms) + self.initial_q
        self.action_count = np.zeros(self.k_arms)

        self.optional_action_freq = 0

    def act(self):
        #explore with prob eps; exploit with prob 1-eps
        if np.random.randn() < self.eps: #explore
    ...
    def reward(self, action_idx):
        #p21: when action a_t is selected, the reward r_t is generated according to a normal distribution with mean q_*(a_t) and variance 1.
        reward = np.random.randn() + self.q_true[action_idx]
        return reward
    ...
    def step(self):
        ...
        # online update of average for optimal action frequency
        if action == np.argmax(self.q_true): ## if action == best possible action
            self.optional_action_freq += 1/np.sum(self.action_count) * (1 - self.optional_action_freq)
        
        return action, reward

class ExponentialAverageBandit(BanditBase):
    def __init__(
            self,
            step_size=0.1,
            **kwargs
...
class GradientBandit(BanditBase):
    """
...
def fig_2_2(runs=2000, steps=1000, epsilons=[0, 0.01, 0.1]):
    bandits = [BanditBase(eps=eps) for eps in epsilons]
...
def fig_2_4(runs=2000, steps=1000):
    bandits = [UCBBandit(eps=0, c=2), BanditBase(eps=0.1)]
